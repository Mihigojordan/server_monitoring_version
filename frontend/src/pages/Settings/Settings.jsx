...
import { getUserId } from '../../lib/adminIdentity'
import { logActivity, getLog } from '../../hooks/useActivityLog'

const DEFAULT_PROFILE = {
  // 1. Profile Information (fullName/profilePicture/phoneNumber/officeLocation
  // are mirrored from the real backend account — see the sync effect below)
  fullName: '', profilePicture: '', employeeId: '', jobTitle: '', department: '',
  ...
  // 4. Permissions & Roles
  teamsManaged: '', approvalRights: false,
  // 5. Notifications
  ...
  companyName: '', department2: '', officeBranch: '', manager: '', teamMembers: '', employeeStatus: 'Active',
...
export default function Settings() {
  const { rawUser, refreshUser, firebaseUser, logout, user } = useApp()
  const userId = getUserId(firebaseUser, rawUser)
  const { value: profile, setValue: setProfile, save } = useFirestoreDoc('operatorProfiles', userId, DEFAULT_PROFILE)
  ...
  // The 4 fields below live on the real backend account — mirror them in
  // from rawUser once it loads so this page shows live data, not stale cache.
  useEffect(() => {
    if (!rawUser) return
    setProfile((p) => ({
      ...p,
      fullName: rawUser.fullName || p.fullName,
      phoneNumber: rawUser.phoneNumber || p.phoneNumber,
      officeLocation: rawUser.address || p.officeLocation,
      profilePicture: rawUser.imageLink || p.profilePicture,
      companyName: rawUser.taxPayerName || p.companyName,
      officeBranch: rawUser.branchName || rawUser.branchId || p.officeBranch,
    }))
  ...
  // Profile Information dual-writes: the 4 real fields go to the actual
  // backend account, the whole doc (including the new spec fields) goes to
  // Firestore.
  async function persistProfileInfo() {
    setSaving(true)
    setProfileErr(null)
    try {
      if (rawUser) {
        await authApi.editUser({
          ...
          address: profile.officeLocation,
      ...
      setSaved(true)
      logActivity({ action: 'UPDATE_PROFILE', category: 'System', summary: 'Updated profile information', status: ok ? 'ok' : 'error' })
  ...
  function exportPersonalData() {
    const blob = new Blob([JSON.stringify({ ...profile, rawUser }, null, 2)], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `infra-profile-${userId}.json`
        ...
        <div className="profile-header">
          <div className="profile-header__avatar">
            {profile.profilePicture || firebaseUser?.photoURL
              ...
              : (profile.fullName || user?.name || 'U').slice(0, 2).toUpperCase()}
          </div>
          <div>
            <div style={{ fontWeight: 800, fontSize: 17, color: 'var(--ink-900)' }}>
              {profile.fullName || rawUser?.taxPayerName || user?.name || 'Device Operator'}
            </div>
            <div style={{ fontSize: 13, color: 'var(--ink-500)' }}>
              {profile.jobTitle || 'Device Operator'} · {profile.email || rawUser?.tin || '—'}
            </div>
          </div>
          <span className="chip chip--brand" style={{ marginLeft: 'auto' }}>
            {rawUser?.isTrainingMode ? 'Training Mode' : 'Operator'}
          </span>
        </div>
        ...
        <div className="profile-layout">
          ...
          <div>
            ...
            {section === 'Profile Information' && (
              <Section title="Profile Information" subtitle="How you appear across InfraMonitor — name, phone and photo sync with your real account">
                ...
                <SettingRow label="Job Title">
                  <input className="form-input form-input--sm" style={{ width: 220 }} value={profile.jobTitle} onChange={set('jobTitle')} placeholder="Device Operator" />
                </SettingRow>
                <SettingRow label="Department">
                  <input className="form-input form-input--sm" style={{ width: 220 }} value={profile.department} onChange={set('department')} placeholder="IT Operations" />
                </SettingRow>
                ...
                <SaveBtn saving={saving} saved={saved} onClick={persistProfileInfo} />
              </Section>
            )}
                ...
                <div style={{ marginTop: 18, paddingTop: 14, borderTop: '1px solid var(--ink-100)' }}>
                  ...
                  <ul className="profile-list">
                    {profile.apiKeys.map((k) => (
                      <li key={k.id}>
                        <span className="k">{k.label} · <span style={{ fontFamily: 'var(--font-mono)' }}>{k.key}</span></span>
            ...
            {section === 'Permissions & Roles' && (
              <Section title="Permissions & Roles" subtitle="What this account can do">
                <SettingRow label="Current Role">
                  <span className="chip chip--brand">Device Operator</span>
                </SettingRow>
                <SettingRow label="Access Level">
                  <span className="chip chip--ok">{rawUser?.branchName || rawUser?.branchId ? `Branch: ${rawUser.branchName || rawUser.branchId}` : 'Standard'}</span>
                </SettingRow>
                <SettingRow label="Assigned Permissions">
                  <span style={{ fontSize: 12.5, color: 'var(--ink-600)' }}>Sales · Invoices · Stock · Purchases · Customers</span>
                </SettingRow>
                <SettingRow label="Teams Managed">
                  <input className="form-input form-input--sm" style={{ width: 220 }} value={profile.teamsManaged} onChange={set('teamsManaged')} placeholder="—" />
                </SettingRow>
                <SettingRow label="Approval Rights" sub="Can approve refunds and price overrides">
            ...
            {section === 'Activity Logs' && (
              <Section title="Activity Logs" subtitle="Recent account and system activity" action={<Link className="btn btn--ghost" to="/activity">View full log</Link>}>
                <ul className="profile-list">
            ...
            {section === 'Organization Information' && (
              <Section title="Organization Information" subtitle="Where this account sits in the org">
                ...
                <SettingRow label="Department">
                  <input className="form-input form-input--sm" style={{ width: 220 }} value={profile.department2} onChange={set('department2')} placeholder="IT Operations" />
                </SettingRow>
                <SettingRow label="Office Branch">
                  <input className="form-input form-input--sm" style={{ width: 220 }} value={profile.officeBranch} onChange={set('officeBranch')} />
                </SettingRow>
            ...
            {section === 'Infrastructure Preferences' && (
              <Section title="Infrastructure Preferences" subtitle="Defaults used across the monitoring platform">
            ...
            {section === 'Documents' && (
              <Section title="Documents" subtitle="Reference links — file storage isn't wired up yet, so these are URLs/notes">
                ...
                <SettingRow label="Certifications">
                  <input className="form-input form-input--sm" style={{ width: 260 }} value={profile.certifications} onChange={set('certifications')} placeholder="e.g. POS operations" />
                </SettingRow>
            ...
            {section === 'Admin Statistics' && (
              <Section title="Admin Statistics" subtitle={stats ? 'Live from the backend' : statsError ? 'Backend unavailable — showing no data' : 'Loading…'}>
                {stats ? (
                  ...
                  <div style={{ fontSize: 13, color: 'var(--ink-500)' }}>
                    {statsError ? 'Could not reach the dashboard summary endpoint — start the backend to see live figures.' : 'Loading statistics…'}
                  </div>
                )}
              </Section>
            )}
            ...
            {section === 'Account Management' && (
              <Section title="Account Management" subtitle="Data export and session control">
                ...
                <SettingRow label="Deactivate Account" sub="Contact an admin to deactivate this account">
                  <button className="btn" disabled title="Contact an admin to deactivate this account">Deactivate</button>
                </SettingRow>
                <SettingRow label="Delete Account" sub="Contact an admin to permanently delete this account">
                  <button className="btn btn--danger" disabled title="Contact an admin to delete this account">Delete</button>
                </SettingRow>
              </Section>
            )}
          </div>
