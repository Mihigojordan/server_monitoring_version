...
export default function NetworkDeviceDetail() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'networkDevices', id), snap => {
  ...
  async function setStatus(status, label) {
    try {
      await updateDoc(doc(db, 'networkDevices', id), { status, updatedAt: serverTimestamp() })
      logActivity({ action: 'NETWORK_DEVICE_STATUS_CHANGE', category: 'Network Management', summary: `${label}: ${device.name}` })
    } catch (err) {
  ...
  async function confirmDelete() {
    try {
      await deleteDoc(doc(db, 'networkDevices', id))
      logActivity({ action: 'DELETE_NETWORK_DEVICE', category: 'Network Management', summary: `Deleted network device: ${device.name}` })
      navigate('/network-management')
    } catch (err) {
      setError(err.message)
  ...
  if (loading) {
    return <AppShell><div className="page"><div className="card" style={{ padding: 32, textAlign: 'center', color: 'var(--ink-500)' }}>Loading network device…</div></div></AppShell>
  }
  if (!device) {
    return (
      <AppShell>
        <div className="page">
          <div className="card" style={{ padding: 40, textAlign: 'center', color: 'var(--ink-500)' }}>
            <div style={{ marginBottom: 12 }}>This network device no longer exists.</div>
            <button className="btn btn--primary" onClick={() => navigate('/network-management')}>Back to Network Management</button>
          </div>
        </div>
      </AppShell>
    )
  ...
  return (
    <AppShell>
      <div className="page">
        <div onClick={() => navigate('/network-management')} style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--ink-500)', fontSize: 13, cursor: 'pointer', marginBottom: 14 }}>
        ...
        <div className="card" style={{ padding: 24, marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--ink-500)' }}>
              {device.deviceId || '—'} · {device.ip || '—'} · {device.manufacturer || '—'} {device.model || ''}
            </div>
          </div>
          <div className="profile-menu" ref={actionsRef}>
            ...
            {showActions && (
              <div className="profile-dropdown" style={{ minWidth: 200 }}>
                <button className="profile-dropdown__item" onClick={() => { setShowActions(false); navigate('/network-management') }}>Edit device</button>
                ...
                <button className="profile-dropdown__item" onClick={() => { setTab('performance'); setShowActions(false) }}>Log performance snapshot</button>
                <button className="profile-dropdown__item" onClick={() => { setTab('ports'); setShowActions(false) }}>Manage ports</button>
                <button className="profile-dropdown__item" onClick={() => { setTab('alerts'); setShowActions(false) }}>Report alert</button>
                <div className="profile-dropdown__divider" />
        ...
        <div style={{ fontSize: 11.5, color: 'var(--ink-500)', marginBottom: 14, marginTop: -6 }}>
          Actions update this tracking record and are logged to Activity — this system does not have live remote access to your physical network hardware.
        </div>
        ...
        {tab === 'ports' && <PortsTab deviceId={id} />}
        ...
        <div className="modal-backdrop" onClick={() => setDeleteConfirm(false)}>
          <div className="modal" role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            <div className="modal__body" style={{ padding: 26 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--ink-500)', marginBottom: 20 }}>This permanently removes the device and all its tracked ports and alerts. This can't be undone.</div>
