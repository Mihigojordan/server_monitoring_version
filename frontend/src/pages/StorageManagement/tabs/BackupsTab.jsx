...
export default function BackupsTab({ deviceId, device: d }) {
  const { rows, loading, error, setError, add, remove } = useSubcollection('storageDevices', deviceId, 'backups', 'createdAt')
  ...
  const [schedule, setSchedule] = useState({
    backupEnabled: d?.backupEnabled ?? true,
  ...
  async function saveSchedule() {
    ...
    try {
      await updateDoc(doc(db, 'storageDevices', deviceId), schedule)
    } catch (err) {
      setError(err.message)
    } finally {
      ...
      <div className="card">
        <div className="table-wrap">
          {loading ? (
            ...
            <div style={{ padding: 28, textAlign: 'center', color: 'var(--ink-500)', fontSize: 13 }}>No backups recorded for this device yet.</div>
          ) : (
            <table className="data">
              <thead><tr><th>Created</th><th>Type</th><th>Status</th><th>Size</th><th>Location</th><th style={{ textAlign: 'right' }}>Actions</th></tr></thead>
              <tbody>
                {rows.map(row => {
            ...
            <form onSubmit={handleCreate}>
              <div className="modal__body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} placeholder="e.g. offsite, secondary NAS" />
                </div>
          ...
          <div className="modal" role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            <div className="modal__body" style={{ padding: 26 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--ink-500)', marginBottom: 20 }}>
                This system doesn't have live access to run an actual restore on your physical storage device — this only records that a restore from this backup was performed, for your tracking history.
              </div>
