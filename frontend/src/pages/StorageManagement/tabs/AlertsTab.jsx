...
export default function AlertsTab({ deviceId }) {
  const { rows, loading, error, setError, add, update, remove } = useSubcollection('storageDevices', deviceId, 'issues', 'createdAt')
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 14 }}>
        <button className="btn btn--primary" style={{ background: '#2563EB', borderColor: '#2563EB' }} onClick={openAdd}>Report Alert</button>
      </div>
      ...
      <div className="card">
        <div className="table-wrap">
          {loading ? (
            ...
            <div style={{ padding: 28, textAlign: 'center', color: 'var(--ink-500)', fontSize: 13 }}>No alerts reported for this device.</div>
          ) : (
            <table className="data">
              <thead><tr><th>Alert</th><th>Severity</th><th>Status</th><th>Assigned</th><th>Detected</th><th style={{ textAlign: 'right' }}>Actions</th></tr></thead>
              <tbody>
                {rows.map(row => {
      ...
      {showForm && (
        <div className="modal-backdrop" onClick={() => setShowForm(false)}>
          <div className="modal" role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={{ maxWidth: 500 }}>
            <div className="modal__head"><h3>{editingId ? 'Edit Alert' : 'Report Alert'}</h3>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal__body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div className="form-group" style={{ gridColumn: '1/-1' }}>
                  ...
                  <input className="form-input" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. Capacity nearing threshold" />
                </div>
              ...
              <div className="modal__body" style={{ paddingTop: 0, display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                ...
                <button type="submit" className="btn btn--primary" style={{ background: '#2563EB', borderColor: '#2563EB' }} disabled={saving}>{saving ? 'Saving…' : 'Save Alert'}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="modal-backdrop" onClick={() => setDeleteTarget(null)}>
          <div className="modal" role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={{ maxWidth: 380 }}>
            <div className="modal__body" style={{ padding: 26 }}>
              <div style={{ fontSize: 16, fontWeight: 800, marginBottom: 8 }}>Delete alert?</div>
